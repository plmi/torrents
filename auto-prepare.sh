...
# usage ./<program> <source directory> <mount point> <destination directory>
...
find "$SOURCE_DIRECTORY" -type f \( -iname "*.mkv" -o -iname "*.iso" \) | while read -r file; do
  ...
  if [[ $file == *mkv ]]; then
    mediainfo --Output=JSON "$file" > "$OUTPUT_JSON"
  elif [[ $file == *iso ]]; then
    ...
    mediainfo --Output=JSON "$largest_file" > "$OUTPUT_JSON" && \
    sudo umount "$MOUNTPOINT"
  fi
done
...
find "$SOURCE_DIRECTORY" -maxdepth 1 -type d -regex '^\.\/.*-.+$' | xargs -I {} \
  maketorrent --announce "https://torrent-syndikat.org/ts_ann.php" --piece-length 24 \
